...
<div class="container mx-auto px-4 py-8 max-w-2xl">
    ...
    <div class="bg-white rounded-lg shadow p-8">
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                <label for="password" class="block text-sm font-medium text-gray-700 mb-1">Nova Senha <span class="text-gray-500">(opcional)</span></label>
                <input type="password"
                       id="password"
                       name="password"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                ...
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-gray-500">Deixe em branco para manter a senha atual. Mínimo de 8 caracteres se alterar.</p>
            </div>
            ...
            <div>
                ...
                <select id="role"
                        ...
                        x-model="role">
                    <option value="organizador" {{ old('role', $user->role) === 'organizador' ? 'selected' : '' }}>Organizador</option>
                    ...
                    <div class="max-h-48 overflow-y-auto border border-gray-300 rounded-lg p-3 space-y-2">
                        ...
                        <label class="flex items-center hover:bg-gray-50 p-2 rounded cursor-pointer">
                            <input type="checkbox"
                                   name="eventos[]"
                                   value="{{ $evento->id }}"
                                   {{ in_array($evento->id, old('eventos', $user->eventos->pluck('id')->toArray())) ? 'checked' : '' }}
